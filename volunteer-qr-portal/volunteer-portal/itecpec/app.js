...
async function loadVolunteerRank(code) {
    const { data: stats } = await window.sb.from('volunteer_stats_itecpec').select('rank').eq('unique_code', code).single();
...
window.showFullHistory = async () => {
    ...
    const [{ data: att }, { data: tsk }] = await Promise.all([
        window.sb.from('attendance_itecpec').select('*').eq('unique_code', code).order('entry_time', { ascending: false }),
        window.sb.from('tasks_itecpec').select('*').eq('unique_code', code).order('created_at', { ascending: false })
    ]);
