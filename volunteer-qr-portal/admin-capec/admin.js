...
function switchTab(tab) {
    ...
    Object.values(views).forEach(v => {
        if (v && !v.classList.contains('overlay')) v.classList.add('hidden');
    });
...
async function loadDashboard() {
    // Stats
    const { count: activeCount } = await sb.from(`attendance_${SUFFIX}`).select('id', { count: 'exact' }).is('exit_time', null);
    const { count: pendingCount } = await sb.from(`attendance_${SUFFIX}`).select('id', { count: 'exact' }).eq('status', 'pending');
    // For tasks pending, separate? Combined pending
    const { count: pendingTasks } = await sb.from(`tasks_${SUFFIX}`).select('id', { count: 'exact' }).eq('status', 'pending');
    ...
    const { data: hrData } = await sb.from(`attendance_${SUFFIX}`).select('duration_minutes').eq('status', 'approved');
...
async function loadApprovals() {
    ...
    if (typeFilter === 'all' || typeFilter === 'attendance') {
        const { data } = await sb.from(`attendance_${SUFFIX}`).select(`*, volunteers_${SUFFIX}(name)`).eq('status', statusFilter);
        att = (data || []).map(a => ({ type: 'attendance', ...a, volunteer_name: a[`volunteers_${SUFFIX}`]?.name }));
    }

    // Fetch tasks
    let tsk = [];
    if (typeFilter === 'all' || typeFilter === 'task') {
        const { data } = await sb.from(`tasks_${SUFFIX}`).select(`*, volunteers_${SUFFIX}(name)`).eq('status', statusFilter);
        tsk = (data || []).map(t => ({ type: 'task', ...t, volunteer_name: t[`volunteers_${SUFFIX}`]?.name }));
    }
    ...
    const { count: pendingAtt } = await sb.from(`attendance_${SUFFIX}`).select('id', { count: 'exact' }).eq('status', 'pending');
    const { count: pendingTsk } = await sb.from(`tasks_${SUFFIX}`).select('id', { count: 'exact' }).eq('status', 'pending');
...
async function loadLeaderboard() {
    const { data: stats, error } = await sb.from(`volunteer_stats_${SUFFIX.toLowerCase()}`).select('*').order('total_minutes', { ascending: false });
...
window.approve = async (id, type) => {
    await callEdge('approve', { id, type, status: 'approved', approved_by: user.email, org: ADMIN_ORG });
    loadApprovals();
};
...
window.decline = async (id, type) => {
    ...
    if (note === null) return;
    await callEdge('approve', { id, type, status: 'declined', note, approved_by: user.email, org: ADMIN_ORG });
    loadApprovals();
};
...
window.sendWhatsAppApproval = async (target_id, type, name) => {
    try {
        ...
        alert("Error generating action link.");
    }
};

async function loadVolunteers() {
     const { data } = await sb.from(`volunteers_${SUFFIX}`).select('*').order('name');
...
window.viewVolunteerDetail = async (id) => {
    ...
    // Load Stats
    const { data: stats } = await sb.from(`volunteer_stats_${SUFFIX.toLowerCase()}`).select('*').eq('id', id).single();
...
window.switchDetailTab = async (type) => {
    ...
    if (type === 'attendance') {
        ...
        btnAtt.style.background = primaryColor;
        btnTsk.style.background = 'rgba(255,255,255,0.05)';
        viewAtt.classList.remove('hidden');
        viewTsk.classList.add('hidden');

        const { data } = await sb.from(`attendance_${SUFFIX}`).select('*').eq('volunteer_id', currentDetailVolId).order('entry_time', { ascending: false });
        ...
        const primaryColor = getComputedStyle(document.documentElement).getPropertyValue('--primary').trim() || '#22c55e';
        btnTsk.style.background = primaryColor;
        btnAtt.style.background = 'rgba(255,255,255,0.05)';
        viewTsk.classList.remove('hidden');
        viewAtt.classList.add('hidden');

        const { data } = await sb.from(`tasks_${SUFFIX}`).select('*').eq('volunteer_id', currentDetailVolId).order('created_at', { ascending: false });
...
document.getElementById('form-add-vol').addEventListener('submit', async (e) => {
    ...
    // Create volunteer
    const { data, error } = await sb.from(`volunteers_${SUFFIX}`).insert({
...
function setupRealtime() {
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: `attendance_${SUFFIX}` }, (payload) => {
        // Refresh if on relevant tab
        if (activeTab === 'dashboard') loadDashboard();
        if (activeTab === 'approvals') loadApprovals();
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: `tasks_${SUFFIX}` }, (payload) => {
         if (activeTab === 'dashboard') loadDashboard();
         if (activeTab === 'approvals') loadApprovals();
    })
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'audit_log', filter: `org=eq.${ADMIN_ORG}` }, (payload) => {
        if (activeTab === 'dashboard') loadDashboard();
    })
    .subscribe();
}
...
window.loadDailyLogs = async () => {
    ...
    // Fetch Approved Attendance & Tasks
    const { data: att } = await sb.from(`attendance_${SUFFIX}`).select('entry_time, duration_minutes, unique_code').eq('status', 'approved');
                        ...
                        <div style="background:var(--card-bg); padding:10px; border-radius:6px; margin-bottom:8px; border-left:3px solid ${a.status === 'approved' ? 'var(--success-color)' : a.status === 'declined' ? 'var(--danger-color)' : 'var(--warning-color)'};">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <div>
                                    ...
                                    <div style="font-size:0.8rem; color:var(--text-secondary);">${a.duration_minutes || 0} mins &bull; ${a.status}</div>
                                </div>
                                ${a.status === 'pending' ? `
                                    <div style="display:flex; gap:5px;">
                        ...
                        <div style="background:var(--card-bg); padding:10px; border-radius:6px; margin-bottom:8px; border-left:3px solid ${t.status === 'approved' ? 'var(--success-color)' : t.status === 'declined' ? 'var(--danger-color)' : 'var(--warning-color)'};">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <div>
                                    ...
                                    <div style="font-size:0.8rem; color:var(--text-secondary);">${t.minutes || 0} mins &bull; ${t.status}</div>
                                </div>
                                ${t.status === 'pending' ? `
                                    <div style="display:flex; gap:5px;">
...
async function onAdminScan(decodedText) {
    ...
    try {
        ...
        let res = await callEdge('checkin', { code, device_id: 'admin_panel', org: ADMIN_ORG });
        ...
        if (res.success) {
            ...
            // Attempt Checkout
             res = await callEdge('checkout', { code, device_id: 'admin_panel', org: ADMIN_ORG });
             if (res.success) {
